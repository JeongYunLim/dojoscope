...
from .cluster import cluster_agreement, cluster_distance_matrix
...
def build_suite_output(
    ...
    n_clusters_resolved = min(n_clusters, len(traces)) if traces else 0
    raw_cluster = cluster_distance_matrix(dist.raw_matrix, k=n_clusters_resolved)
    res_cluster = cluster_distance_matrix(dist.residual_matrix, k=n_clusters_resolved)
