...
**이벤트 → 벡터 → DTW 시퀀스 거리 → GT 정렬 잔차 → UMAP 2D 지도 → 라벨 검증** —
...
1. **정적 화면**(`viz/`) — `index.html`/`style.css`/`app.js` + `viz/data/*.json`.
   서버 없이 아무 정적 파일 서버로 열면 된다. 흰 배경 + 빨강/파랑/초록/보라
   원색 팔레트.
2. **데이터 추가**(`add_data.py`) — 새 trace JSON 파일 하나를 인자로 주면,
   기존 데이터와 병합하고 파이프라인(임베딩→DTW→GT정렬/잔차→UMAP→검증)을
   다시 돌려서 `viz/data/`를 갱신한다. 끝나면 브라우저에서 새로고침(F5)만
   하면 된다.
3. **파이프라인**(`pipeline/`) — 이벤트 벡터화, GT 대비 DTW 정렬/잔차 추출,
   suite별 거리 행렬, UMAP 2D 투영, 라벨 검증(실루엣 + 순열 검정)을 계산한다.
   `add_data.py`와 `run_pipeline.py`(전체 재계산용) 둘 다 이 파이프라인을
   그대로 재사용한다.
4. **정적 그림**(`scripts/generate_figures.py`) — `viz/data/`의 JSON을 읽어
   논문용 벡터 PDF/PNG 3종(잔차 UMAP 그리드, A→B 화살표 지도, raw/residual
   실루엣 비교)을 생성한다.
5. **샘플 데이터**(`scripts/generate_sample_data.py`) — 실 AgentDojo 로그 없이도
   바로 체험할 수 있도록 4 suite × 조건 A(무방어)/B(tool_filter), 192케이스 합성.
6. **단위 테스트**(`tests/`, pytest, 33개) — DTW·GT 정렬·τ 자동 보정·임베더·
...
상단 "ⓘ suite란?" 버튼을 누르면 banking/slack/travel/workspace가 각각 무엇을
시뮬레이션하는 환경인지 우측 패널에 뜨고, "📖 분석 가이드" 버튼을 누르면 이
지도를 어떤 순서로 읽어야 하는지(원본→잔차→군집 검증→조건 비교→브러시로
파고들기) 실제 1차 실험 수치와 함께 5단계로 안내한다.

화면 기본 기호는 항상 같은 뜻으로 고정되어 있어 범례를 몰라도 읽힌다: 색은
**방어 결과**(파랑=방어 성공, 빨강=뚫림), 모양은 **과제 결과**(O=성공, X=실패).
상단의 "조건 필터"로 무방어/방어(tool_filter)를 각각 켜 보면 같은 지도에서
빨강 점 분포가 어떻게 바뀌는지 비교할 수 있다 — 이전 버전의 "A→B 화살표"
오버레이는 지도를 복잡하게 만든다는 판단하에 지도에서는 제거했고(케이스 상세
뷰의 GT/A/B 3줄 정렬에는 그대로 남아 있다), 대신 이 필터로 대체했다. 데이터
추가(`add_data.py`)는 지금 단계에서 화면에 노출할 필요가 없다고 판단해 UI에서는
뺐다 — 사용법은 아래를 참고.
...
UMAP 지도는 항상 같은 suite 안에서만 계산한다(§1.4). 대시보드의 "ⓘ suite란?"
버튼을 누르면 화면에서도 바로 이 설명을 볼 수 있다.
...
| 흰 배경 + 빨강/파랑/초록/보라 원색 팔레트 | 가독성, 한눈에 구분되는 선명한 색 |
| "ⓘ suite란?" — banking/slack/travel/workspace 설명 패널 | 도메인 이해 |
| "＋ 데이터 추가 방법" — 화면 안에서 바로 안내 | 사용법 발견성 |
...
| 색상: 판정 상태 / 납치 도구 / 지연 / 노출 채널 / 조건 | 다중 시각 인코딩 |
...
| 우측 raw→residual 실루엣 + 순열검정 유의성(★) 표 | PDF 8장 검증표 + 통계적 근거 |
...
- **색상 접근성 트레이드오프.** 빨강/파랑/초록/보라 원색 팔레트는 적록색맹
  (deuteranopia) 사용자에게 빨강·초록 두 판정 상태가 구분하기 어려울 수
  있다 — 색상만으로 판단하지 않도록 모양(조건 A/B)과 툴팁(정확한 라벨)을
  항상 함께 제공하지만, 발표 대상에 색각 이상이 있는 청중이 포함된다면
  Okabe & Ito(2008) 색맹 안전 팔레트로 되돌리는 것을 권장한다
...
5. **브라우저 완전 실시간 반영이 꼭 필요해지면**: (a) 파일 감시(watchdog) +
